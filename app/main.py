...
async def lifespan(app: FastAPI):
    ...
    print("✅ DoctorGuardians 서버 시작 (포트 8001)")
    yield
    print("🛑 DoctorGuardians 종료")


app = FastAPI(
    title="DoctorGuardians",
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[{
                "role": "user",
                "content": [
                    {"type": "text", "text": "이 사진에서 약 이름들만 추출해주세요. 쉼표로 구분해서 약 이름만 나열해주세요. 예: 아스피린, 메트포르민, 아모디핀"},
            ...
            for item in items:
                ...
                if cat == "T1H":
                    ...
                    if temp_f <= 0:
                        ...
                        weather["tips"] = ["🧥 따뜻하게 입고 외출하세요", "🫖 따뜻한 차를 드세요"]
                    elif temp_f <= 10:
                        weather["icon"] = "🍂"
                        weather["description"] = "쌀쌀함"
                        weather["tips"] = ["🧣 겉옷을 챙기세요"]
                    elif temp_f <= 20:
                        weather["icon"] = "🌤️"
                        weather["description"] = "선선함"
                        weather["tips"] = ["🚶 산책하기 좋은 날씨예요"]
                    elif temp_f <= 30:
                        weather["icon"] = "☀️"
                        weather["description"] = "따뜻함"
                        weather["tips"] = ["💧 수분을 충분히 섭취하세요"]
                    else:
                        weather["icon"] = "🔥"
                        weather["description"] = "매우 더움"
                        weather["tips"] = ["🚫 야외 활동을 자제하세요", "💧 수분 섭취 필수"]
                elif cat == "REH":
...
async def system_status():
    """서버 상태 확인"""
    return JSONResponse({"status": "ok", "message": "DoctorGuardians 서버 정상"})
